...
static char *fts_get_vendor_name(int vendor_id)
{
	switch(vendor_id){
		...
		case FTS_CTP_VENDOR_BOE:	return "BOE";		break;
...
static  int update_firmware_thread(void *priv)
{
    ...
    work_lock=1;
	if( 0 == memcmp(tpd_desc, "BOE", 3))
	{
		ft6x06_tp_upgrade(ftbin_boe, sizeof(ftbin_boe));
	}
    work_lock = 0;
	kthread_should_stop();
	return NULL;
	...
	CTP_DBG("tpd_suspend bEnTGesture:%d\n",bEnTGesture);
	   if (1==bEnTGesture){
		   ...
		   tpd_gesture_cmd();	//20150207
			...
			fts_6x06_power_down();
		#endif
        		disable_irq(touch_irq);//BUG<REQ><BGAFM-80><20160227>add tp gesture for P4605;xiongdajun
