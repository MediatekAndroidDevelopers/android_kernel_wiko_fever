...
static u8 g_proximity_en = 0;
	...
	{
		#include "LIDE_GQ3030_LONGWEI_C02_2016-04-22_1837_modified.i"
...
{
	int ret;
	ts->input_dev =tpd->dev;// input_allocate_device();
...
//	input_set_abs_params(ts->input_dev, ABS_MT_POSITION, 0, (BIT(31) | (ts->pdata->abs_x_max << 16) | ts->pdata->abs_y_max), 0, 0);
	...
	return 0;//input_register_device(ts->input_dev);
}
...
//	int gest_len;  
	...
	if(buf[GEST_PTLG_ID_LEN] != GEST_PTLG_HDR_ID1 || 
			...
			{
				printk("HIMAX wengwentian return 1");
				return 0;
		}
	for(i=0;i<(GEST_PTLG_ID_LEN+GEST_PTLG_HDR_LEN);i++)
	{
		printk("P[%x]=0x%2.2X \n", i, buf[i]);
		printk("checksum=0x%2.2X \n", check_sum_cal);
		check_sum_cal += buf[i];		
	}
	if ((check_sum_cal != 0x00) )
	{
		printk(" %s : check_sum_cal: 0x%02X\n",__func__ ,check_sum_cal);
		return 0;
	}
		...
		for (i=0; i<3; i++)
			{
				...
				gest_pt_cnt +=1;
				printk("gest_key_pt_x[%d]=%d \n ", i, gest_key_pt_x[i]);
				printk("gest_key_pt_y[%d]=%d \n ", i, gest_key_pt_y[i]);
			}
		//parse all point location
		i = 0;
		gest_pt_cnt = 0;
		printk("gest doornidate start \n %s",__func__);
		...
		if(gest_pt_cnt)
			{
				...
				printk("gest_point x_min= %d, x_max= %d, y_min= %d, y_max= %d\n",tmp_min_x,tmp_max_x,tmp_min_y,tmp_max_y);
				...
				printk("gest_start_x= %d, gest_start_y= %d, gest_end_x= %d, gest_end_y= %d\n",gest_start_x,gest_start_y,gest_end_x,gest_end_y);
			}
	...
	if(gesture_flag != 0x80)
	{
		if(!ts->gesture_cust_en[gesture_flag])
			{
				printk("%s NOT report customer key \n ",__func__);
	...
	else
	{
		if(!ts->gesture_cust_en[0])
			{
				printk("%s NOT report report double click \n",__func__);
		...
		if ( tp_key_index == 0x01) {
			...
			printk("back key pressed\n");
				if (1)//ts->pdata->virtual_key
				{
					...
					x_position=tpd_dts_data.tpd_key_dim_local[2].key_x;		
					y_position=tpd_dts_data.tpd_key_dim_local[2].key_y;	
...
//							100);
			...
			I("home key pressed\n");
				if(1)//(ts->pdata->virtual_key)
				{
				    ...
				    x_position=tpd_dts_data.tpd_key_dim_local[1].key_x;		
					y_position=tpd_dts_data.tpd_key_dim_local[1].key_y;	
						if (ts->protocol_type == PROTOCOL_TYPE_A) {
						input_report_abs(ts->input_dev, ABS_MT_TRACKING_ID, 0);
						input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR,
							100);
			...
			I("APP_switch key pressed\n");
				if(1)//(ts->pdata->virtual_key)
				{
					if (ts->button[2].index) {
					  ...
					  x_position=tpd_dts_data.tpd_key_dim_local[0].key_x;		
					  y_position=tpd_dts_data.tpd_key_dim_local[0].key_y;	
						if (ts->protocol_type == PROTOCOL_TYPE_A) {
						input_report_abs(ts->input_dev, ABS_MT_TRACKING_ID, 0);
						input_report_key(ts->input_dev, BTN_TOUCH, 1);
						input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR,
...
#ifdef HX_SMART_WAKEUP	
		private_ts->SMWP_enable = tpd_gesture_enable_status; // 1
...
{	
	...
	struct himax_ts_data *ts;
	struct himax_i2c_platform_data *pdata;
	printk("[Himax] frank_zhonghua:Himax_probe_start\n");
        printk("[Himax] frank_zhonghua1:%s,%x\n",__func__,client->addr);

     
	//Check I2C functionality
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		printk("[Himax] %s: i2c check functionality error\n", __func__);
		err = -ENODEV;
	...
	if(!gpDMABuf_va)
	{
		printk("[Himax] Allocate DMA I2C Buffer failed\n");
	...
	if (ts == NULL) {
		printk("[Himax] %s: allocate himax_ts_data failed\n", __func__);
		err = -ENOMEM;
		goto err_alloc_data_failed;
	}
printk("[Himax] junpiao: dma_alloc_coherent\n");
	
	i2c_set_clientdata(client, ts);
	printk("[Himax] frank_zhonghua1_himax_client-addr = %x\n",client->addr);
	ts->client = client;
	if(ts->client-> addr !=0x48)
	{
	ts->client-> addr =0x48;
	printk("[Himax] frank_zhonghua2_himax_client-addr = %x\n",client->addr);
		}
...
printk("[Himax] junpiao: i2c_set_clientdata\n");

#ifdef CONFIG_OF
	if (client->dev.of_node) { /*DeviceTree Init Platform_data*/
		err = himax_parse_dt(ts, pdata);
		if (err < 0) {
			printk(" [Himax] himax pdata is NULL for DT\n");
	...
	printk("[Himax]  %s:panel-coords = %d, %d, %d, %d\n", __func__, pdata->abs_x_min,
			pdata->abs_x_max, pdata->abs_y_min, pdata->abs_y_max);
	
	//Set display coordinates
	pdata->screenWidth  = hx_display_coords[1];
	pdata->screenHeight = hx_display_coords[3];
	printk("[Himax]  %s:display-coords = (%d, %d)", __func__, pdata->screenWidth,
	pdata->screenHeight);
	//report type
...
#ifdef HX_RST_PIN_FUNC
	ts->rst_gpio = himax_tpd_rst_gpio_number;//pdata->gpio_reset;
#endif
...
himax_gpio_power_config(ts->client);
	...
	if (pdata->power) {
		err = pdata->power(1);
		if (err < 0) {
			printk("[Himax] %s: power on failed\n", __func__);
	...
	//	printk("[Himax] Himax chip doesn NOT EXIST");
	...
	if (pdata->virtual_key)
		...
		if (!ts->flash_wq)
		{
			printk("[Himax] %s: create flash workqueue failed\n", __func__);
			err = -ENOMEM;
	...
	if(i_update_FW()==false)//****************************************************fw auto update
		printk("NOT Have new FW=NOT UPDATE=\n");
	else
		printk("Have new FW=UPDATE=\n");
#endif
	...
	if (himax_loadSensorConfig(client, pdata) < 0) {
		printk("%s: Load Sesnsor configuration failed, unload driver.\n", __func__);
	...
	if (getMutualBuffer() == NULL) {
		printk("%s: mutual buffer allocate fail failed\n", __func__);
	...
	if(Is_2T2R){
		...
		if (getMutualBuffer_2() == NULL) {
			printk("%s: mutual buffer 2 allocate fail failed\n", __func__);
			goto err_setchannel_failed;
		}
	}
#endif	
...
#endif
#ifdef HX_SMART_WAKEUP
	ts->SMWP_enable = tpd_gesture_enable_status; // 1
...
printk("[Himax] frank_zhonghua:Himax_probe_end\n");
return 0;
...
//if (err != 0)
//	printk("[Himax] Failed to disable reg-vgp6: %d\n", err);
...
void himax852xes_suspend(struct device *dev)
{
	...
	{
		printk("%s: Driver probe fail. \n", __func__);
		return;
	}
	printk("%s: Enter suspended. \n", __func__);
...
#if defined(CONFIG_TOUCHSCREEN_PROXIMITY)
	if(ts->pdata->proximity_bytp_enable){
			printk("[Proximity],Proximity en=%d\r\n",g_proximity_en);
			if(g_proximity_en)
	...
	{
		printk("[himax] %s: Flash dump is going, reject suspend\n",__func__);
		return;
	}
#endif
#ifdef HX_TP_PROC_HITOUCH
	...
	{
		printk("[himax] %s: Hitouch connect, reject suspend\n",__func__);
		return;
	}
#endif
...
#ifdef HX_SMART_WAKEUP
	ts->SMWP_enable = tpd_gesture_enable_status;
	if(ts->SMWP_enable)
	{
		atomic_set(&ts->suspend_mode, 1);
		ts->pre_finger_mask = 0;
		...
		printk("[himax] %s: SMART_WAKEUP enable, reject suspend\n",__func__);
		return;
	}
#endif
...
{
	//int retval;
	printk("[Himax] Himax_ts I2C Touchscreen Driver local init\n");
    ...
    {
        printk("[Himax]  unable to add i2c driver.\n");        
        return -1;
    }
...
static void __init himax852xes_init_async(void *unused, async_cookie_t cookie)
{
	...
	if (tpd_driver_add(&tpd_device_driver) < 0)
		printk("add hX852Xes driver failed\n");
#endif
...
static int __init himax852xes_init(void)
{
	...
	tpd_get_dts_info();
#ifdef TPD_HAVE_BUTTON	
		tpd_dts_data.tpd_resolution[0]=1080;
		tpd_dts_data.tpd_resolution[0]=1920;
		tpd_dts_data.touch_filter.pixel_density=124;
	    tpd_dts_data.use_tpd_button=1;
		tpd_dts_data.tpd_key_local[0]=KEY_MENU;
		tpd_dts_data.tpd_key_local[1]=KEY_HOMEPAGE;
		tpd_dts_data.tpd_key_local[2]=KEY_BACK;
		tpd_dts_data.tpd_key_dim_local[0].key_x=245;//{{100,1000,20,40}, {250,1000,20,40}, {400,1000,20,40}};
		tpd_dts_data.tpd_key_dim_local[0].key_y=2000;
		tpd_dts_data.tpd_key_dim_local[0].key_width=20;
		tpd_dts_data.tpd_key_dim_local[0].key_height=20;
		tpd_dts_data.tpd_key_dim_local[1].key_x=540;//{{100,1000,20,40}, {250,1000,20,40}, {400,1000,20,40}};
		tpd_dts_data.tpd_key_dim_local[1].key_y=2000;
		tpd_dts_data.tpd_key_dim_local[1].key_width=20;
		tpd_dts_data.tpd_key_dim_local[1].key_height=20;
		tpd_dts_data.tpd_key_dim_local[2].key_x=835;//{{100,1000,20,40}, {250,1000,20,40}, {400,1000,20,40}};
		tpd_dts_data.tpd_key_dim_local[2].key_y=2000;
		tpd_dts_data.tpd_key_dim_local[2].key_width=20;
		tpd_dts_data.tpd_key_dim_local[2].key_height=20;
		tpd_dts_data.tpd_key_num=3;
