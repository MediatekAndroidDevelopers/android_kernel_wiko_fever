...
MODULE_DEVICE_TABLE(of, epl259x_of_match);
#endif


/* Maintain alsps cust info here */

static struct alsps_hw epl259x_cust = {
    .i2c_num    = 2,
    .polling_mode_ps = 1,
    .polling_mode_als = 1,
...
static struct alsps_hw *hw = &epl259x_cust;
static struct platform_device *alspsPltFmDev;

/* For alsp driver get cust info */
struct alsps_hw *epl259x_get_cust_alsps(void)
{
	return &epl259x_cust;
}

//#define EPL259X_DEBUG
#ifdef  EPL259X_DEBUG
#define APS_TAG                 	  	"[EPL259X] "
...
static struct i2c_driver epl_sensor_i2c_driver =
{
    ...
    .driver = {
        .name   = EPL_DEV_NAME,
#ifdef CONFIG_OF
		.of_match_table = epl259x_of_match,
#endif			
    },
};
...
static irqreturn_t epl_sensor_eint_handler(int irq, void *desc)
{
...
{	
	...
	epl_sensor_obj->irq_node = of_find_compatible_node(NULL, NULL, "mediatek, als-eint");
	/* eint request */
	if (epl_sensor_obj->irq_node) {
		...
		if (request_irq(epl_sensor_obj->irq, epl_sensor_eint_handler, IRQF_TRIGGER_NONE, "ALS-eint", NULL)) {
...
/*----------------------------------for factory mode test---------------------------------------*/
		case ALSPS_GET_PS_TEST_RESULT:
			...
			if(dat > obj->hw->ps_threshold_high)
			{
			    ...
			    ps_result = 1;
			*/
			ps_result = epl_sensor.ps.compare_low >> 3;
...
static int epl_sensor_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if((i2c_smbus_read_byte_data(client, 0x21)) != EPL_REVNO){ //check chip
        printk("elan ALS/PS sensor is failed. \n");
        err = -1;
        goto exit;
    }
...
{
    struct alsps_hw *hw = epl259x_get_cust_alsps();
...
static int __init epl_sensor_init(void)
{
	...
	if (!hw)
		APS_ERR("epl269x get dts info fail\n");
