...
static const struct of_device_id stk3x1x_of_match[] = {
	{ .compatible = "mediatek,mt6580-stk3x1x", },
	{},
};

MODULE_DEVICE_TABLE(of, stk3x1x_of_match);
#endif

/* Maintain alsps cust info here */
struct alsps_hw stk3x1x_cust;
static struct alsps_hw *hw = &stk3x1x_cust;
static struct platform_device *alspsPltFmDev;

/* For alsp driver get cust info */
struct alsps_hw *stk3x1x_get_cust_alsps(void)
{
	return &stk3x1x_cust;
}
...
int stk3x1x_setup_eint(struct i2c_client *client)
{
	...
	err = SCP_sensorHub_rsp_registration(ID_PROXIMITY, stk3x1x_eint_handler);
...
/* gpio setting */
	...
	stk3x1x_obj->irq_node = of_find_compatible_node(NULL, NULL, "mediatek, als-eint");
	/* eint request */
...
static int stk3x1x_local_init(void) 
{
	struct alsps_hw *hw = stk3x1x_get_cust_alsps();
