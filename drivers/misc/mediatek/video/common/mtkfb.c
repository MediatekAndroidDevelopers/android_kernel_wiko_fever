...
int is_ili7802_hd720_dsi_vdo_boe = 0;//zhangchongyong add 20160412 for ili7802 esd
int is_hx8394f_dsi_vdo_auo = 0;//xiehaifei add 20160627 for hx8394f esd
...
int _parse_tag_videolfb(void)
{
	...
	if (of_scan_flat_dt(fb_early_init_dt_get_chosen, NULL) > 0) {
		...
		if (videolfb_tag) {
			...
			/*zhangtao add for lcm factory ata test*/
			if (!strcmp(mtkfb_lcm_name, "ili7802_hd720_dsi_vdo_boe"))  is_ili7802_hd720_dsi_vdo_boe = 1;
			if (!strcmp(mtkfb_lcm_name, "hx8394f_dsi_vdo_auo"))  is_hx8394f_dsi_vdo_auo = 1;
			if(islcmconnected)
...
static int subsys_lcd_info_read(struct seq_file *m, void *v)
{
   seq_printf(m, "%s\n",mtkfb_lcm_name);
   return 0;
};

static int proc_lcd_info_open(struct inode *inode, struct file *file)
{
    return single_open(file, subsys_lcd_info_read, NULL);
};

static  struct file_operations lcd_proc_fops = {
    .owner = THIS_MODULE,
    .open  = proc_lcd_info_open,
    .read  = seq_read,
};
#endif
...
#ifdef LYCONFIG_DETECT_HW_INFO_SUPPORT
	/* Lcd information */
	proc_create("hw_info/lcd_info", 0, NULL, &lcd_proc_fops);
#endif
